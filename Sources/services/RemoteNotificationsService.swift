...
public class RemoteNotificationsService: Observable {
    ...
    invalidateAuthorizationStatus { status in
      ...
      case .notDetermined:
        UNUserNotificationCenter.current().requestAuthorization(options:[.badge, .alert, .sound]) { granted, error in
          if let error = error {
            _log.error { "Requesting for remote notifications authorization... ERR\n↘︎ error=\(error)" }
            return
          }
      ...
      case .authorized:
        _log.debug { "Requesting for remote notifications authorization... SKIP\n↘︎ Already granted" }
      }
    }
  }
  ...
  public func invalidatePushToken() {
    invalidateAuthorizationStatus { status in
      guard status == .authorized else {
        _log.debug { "Invalidating push token... SKIP\n↘︎ Authorization status should be \(AuthorizationStatus.authorized), currently \(status)" }
        return
      }
  ...
  public func didRegisterForRemoteNotificationsWithDeviceToken(_ token: Data) {
    ...
    _log.debug { "Invalidating push token... OK\n↘︎ token=\(tokenString)" }
  ...
  public func didFailToRegisterForRemoteNotificationsWithError(_ error: Error) {
    _log.error { "Invalidating push token... ERR\n↘︎ error=\(error)" }
